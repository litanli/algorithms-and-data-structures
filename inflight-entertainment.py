...
# there's two movies whose total runtime equals exactly the flight time.
...
# Can we decrease runtime to linear time? Let's try to do away with the inner
# for loop. The exact length of the second movie can be determined. We can turn
# our movie lengths into a set, then look up the set for the length we want in
# constant time. The total runtime is O(n). O(n) space.
def can_two_movies_fill_flight(flight_length, movie_lengths):
...
# loop through the movie lengths only once, taking a constant time in each 
# iteration. Because of their constant lookup time, using hash-based data structures, like dictionary or sets, is so
# common in coding challenge solutions that you should always ask yourself from
# the start "Can I save time by using a dictionary?"